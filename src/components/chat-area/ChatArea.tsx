...
import UserInfoModal from "../chat-user-info-modal/UserInfoModal";
...
export default function ChatArea({
  ...
  const [chosenUser, setChosenUser] = useState<{ [key: string]: any }>({});

  const [space, setSpace] = useState(false);

  /*
   * @param timeIsOut needed to improve UX (ChatArea is rendered before ChatList on small-screen devices, which causes bad UX)
   */
  ...
  useEffect(() => {
    timer(350).subscribe(() => setTimeIsOut(true));
  }, []);

  useEffect(() => {
    if (listRef.current && newMessage) {
      timer(50).subscribe(() => listRef.current.scrollIntoView({ behavior: "smooth", block: "end", inline: "end" }));
    }
  }, [newMessage]);
  ...
  useEffect(() => {
    const observer = new IntersectionObserver((entries, observer) => {
      entries.forEach(
        (entry) => {
          if (entry.isIntersecting && editing && !space) {
            setSpace(true);
    ...
    if (editing && lastElRef.current) {
      if (isInViewport(lastElRef.current)) {
        setSpace(true);
      }
  ...
  useEffect(() => {
    if (space && listRef.current) listRef.current.scrollIntoView({ behavior: "smooth", block: "end", inline: "end" });
  }, [space]);
          ...
          {!chatName ? (
            <section className="chat-layout flex a-i-c j-c-f-s f-f-c-n">
              <svg
                ...
                className="f-w f-h a-s-c j-s-c dummy-svg"
                width="863.91732"
                height="364.20537"
              ...
              >
                <polygon
                  ...
                  fill="#f0f0f0"
                />
                <rect x="8.66553" y="129.71814" width="294.62811" height="81.92868" fill="#fff" />
                <rect x="34.72148" y="154.42552" width="141.85589" height="4.30497" fill="#f0f0f0" />
                <rect x="34.72148" y="169.31777" width="247.24292" height="4.30497" fill="#f0f0f0" />
                <rect x="34.72148" y="184.21003" width="247.00081" height="4.30497" fill="#f0f0f0" />
                <path d="M692.07274,618.96676s1.487-31.15875,31.97119-27.537" transform="translate(-168.04134 -267.89732)" fill="#f0f0f0" />
                <circle cx="515.41796" cy="306.16087" r="15.2571" fill="#f0f0f0" />
                <rect x="512.9354" y="331.85294" width="4.30672" height="30.14703" fill="#f0f0f0" />
                ...
                <circle cx="578.08341" cy="210.89752" r="14.50548" fill="#dfbeff" />
                <circle cx="744.8965" cy="210.89752" r="14.5055" fill="#dfbeff" />
                <polygon points="661.49 181.886 650.611 229.029 668.742 210.898 661.49 181.886" fill="#dfbeff" />
                <polygon
                ...
                <polygon
                  ...
                  fill="#cacaca"
                />
                <rect x="68.26381" y="14.39335" width="425.22943" height="118.24561" fill="#fff" />
                <rect x="102.45877" y="48.91591" width="204.73707" height="6.21326" fill="#dfbeff" />
                <rect x="102.45877" y="70.40954" width="356.83952" height="6.21326" fill="#dfbeff" />
                <rect x="102.45877" y="91.90316" width="356.49009" height="6.21326" fill="#dfbeff" />
                <path
              ...
              <h1 className="h2-s t-c dummy">{t("chatArea.header")}</h1>
            </section>
          ) : (
            <section className={`chat-layout grid ${expand ? "expanded" : ""}`} role="log" aria-live="polite">
              <ChatHeader socketRef={socketRef} membersCount={2} activeMembersCount={1} />
              <div className="messages-container">
                <ul className="messages-list flex j-c-f-e f-f-c-n" ref={listRef} style={space ? { paddingBottom: "100px" } : undefined}>
                  {messages &&
                    messages.map((item: any, index: number) => (
                      <li key={item._id} id={item._id} ref={index === messages.length - 1 ? lastElRef : undefined}>
                        <Message
              ...
              <ChatInput socketRef={socketRef} expand={expand} setExpand={setExpand} editing={editing} setEditing={setEditing} />
            </section>
          )}
          <UserInfoModal user={chosenUser} />
        </React.Fragment>
      ) : null}
    </React.Fragment>
  );
